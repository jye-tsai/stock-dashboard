...
- **總覽**:總報酬甜甜圈(未實現 / 已實現 / 股息)、市值比例甜甜圈、年度損益、資產走勢、每日市值變動、報酬對比大盤、持股損益。
...
三圖共用同一組時間範圍(近 1 月 / 近 3 月 / 全部)與同一份 `hist`;**週末**一律不畫,**有 taiex 之後**沒 taiex 的日期(國定假日休市)也不畫。
...
2. **GitHub Token**(fine-grained PAT,只給此 repo):`Contents: Read and write`(存檔)+ `Actions: Read and write`(觸發 workflow_dispatch)。用於儀表板「⚙️ GitHub」同步與 Cloudflare 的 `GH_TOKEN`。
...
- 密碼雜湊用 **PBKDF2-SHA256 + 隨機鹽**(解鎖後可按「🔑 設定密碼」);相容舊版 SHA-256。
